    ...
    extends AbstractModule
{
    ...
    /**
     * Part of the EDSL builder language for configuring {@code Job}s. Here is a typical example of scheduling
     * {@code Job}s when creating your Guice injector:
     *
     * <pre>
    ...
    /**
     * Allows {@code Job} scheduling, delegating Guice create the {@code Job} instance and inject members. If given
     * {@code Job} class is annotated with {@link Scheduled}, then {@code Job} and related {@code Trigger} values will
    ...
    protected final JobSchedulerBuilder scheduleJob( Class<? extends Job> jobClass )
    {
        ...
        if ( jobClass.isAnnotationPresent( Scheduled.class ) )
        {
            ...
            builder
            // job
            .withJobName( scheduled.jobName() ).withJobGroup( scheduled.jobGroup() ).withRequestRecovery( scheduled.requestRecovery() ).withStoreDurably( scheduled.storeDurably() )
            // trigger
            .withCronExpression( scheduled.cronExpression() ).withTriggerName( scheduled.triggerName() );
     ...
     *
     * @param <T> generic representing type of the class
     * @param binder objects where bindings should be added
     * @param type the class to be binded
     */
