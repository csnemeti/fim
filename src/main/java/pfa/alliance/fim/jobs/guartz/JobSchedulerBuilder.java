package pfa.alliance.fim.jobs.guartz;
...
/**
 * DSL to produce {@code Job} and add to a {@code Scheduler}, and associate the
 * related {@code Trigger} with it.
 */
public final class JobSchedulerBuilder {
	...
	/**
	 * Instructs the {@code Scheduler} whether or not the {@code Job} should be
	 * re-executed if a {@code recovery} or {@code fail-over} situation is
	 * encountered.
	 */
	private boolean requestRecovery = false;

	/**
	 * Whether or not the {@code Job} should remain stored after it is orphaned
	 * (no {@code Trigger}s point to it).
	 */
	...
	/**
	 * The time zone for which the {@code cronExpression} of this
	 * {@code CronTrigger} will be resolved.
	 */
	private TimeZone timeZone = getDefault();

	/**
	 * The {@code Trigger}'s priority. When more than one {@code Trigger} have
	 * the same fire time, the scheduler will fire the one with the highest
	 * priority first.
	 */
	...
	/**
	 * Indicates whether the job's trigger should be updated if it is already
	 * existing when being scheduled (which is typically the case with a
	 * persistent {@link org.quartz.spi.JobStore}.
	 *
	 * @since 1.3
	 */
	...
	/**
	 * Sets the {@code Trigger}'s priority. When more than one {@code Trigger}
	 * have the same fire time, the scheduler will fire the one with the highest
	 * priority first.
	 *
	 * @param priority
	 *            The {@code Trigger}'s priority
	 * @return This builder instance
	 */
	...
	/**
	 * Sets the {@code Trigger} that will be used to schedule the {@code Job}.
	 *
	 * <p>
	 ...
	 *
	 * @param trigger
	 *            The {@code Trigger} to associate with the {@code Job}
	 * @return This builder instance
	 * @since 1.2
	 */
	...
	/**
	 * Requests an existing trigger (sharing the same key as the new trigger)
	 * for this job to be replaced with the new trigger.
	 *
	 * @return This builder instance
	 * @since 1.3
	 */
	...
	/**
	 * Add the produced {@code Job} to the given {@code Scheduler}, and
	 * associate the related {@code Trigger} with it.
	 *
	...
	public void schedule(Scheduler scheduler) throws Exception {
		if (cronExpression == null && trigger == null) {
			throw new ProvisionException(format(
					"Impossible to schedule Job '%s' without cron expression",
					jobClass.getName()));
		}
		if (cronExpression != null && trigger != null) {
			throw new ProvisionException(format(
					"Impossible to schedule Job '%s' with cron expression "
							+ "and an associated Trigger at the same time",
					jobClass.getName()));
		}

		JobKey jobKey = jobKey(DEFAULT.equals(jobName) ? jobClass.getName()
				: jobName, jobGroup);
		TriggerKey triggerKey = triggerKey(
				DEFAULT.equals(triggerName) ? jobClass.getCanonicalName()
		...
		scheduler.scheduleJob(
				newJob(jobClass).withIdentity(jobKey)
						.requestRecovery(requestRecovery)
						.storeDurably(storeDurably).build(),
				(trigger == null) ? newTrigger()
						.withIdentity(triggerKey)
						.withSchedule(
								cronSchedule(cronExpression).inTimeZone(
										timeZone)).withPriority(priority)
						.build() : trigger);
	}
